...
class CeCircle(_Loss):
    def __init__(self, ce_weight):
        super(CeCircle, self).__init__()

        self.ce = nn.CrossEntropyLoss(ce_weight)
        self.circ = CircleLoss()

    def forward(self, input, feature, target):
        celoss = self.ce(input[target >= 0], target[target >= 0])
        circle = 0.5 * self.circ(feature, target)
        return celoss + circle
